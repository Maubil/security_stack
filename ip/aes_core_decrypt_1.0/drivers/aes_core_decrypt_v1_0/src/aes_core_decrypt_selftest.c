...
XStatus AES_CORE_DECRYPT_SelfTest(void * baseaddr_p)
{
	...
	aes_core_decrypt_set_addr((u32) baseaddr_p);

	xil_printf("************************************\n\r");
	xil_printf("* AES DECRYPT Peripheral Self Test *\n\r");
	xil_printf("************************************\n\n\r");
	...
	if (aes_core_decrypt_init_key(key) != XST_SUCCESS)
	{
		return XST_FAILURE;
	}
	...
	{
		u32 result[4] = {};
		XStatus rc = aes_core_decrypt(ciphertext[i], result);

		if (rc != XST_SUCCESS || memcmp(result, plaintextes[i], sizeof(u32) * 4) != 0)
		{
			xil_printf("Failed at iteration %u\n\rExpected 0x%08x %08x %08x %08x\n\rGot 0x%08x %08x %08x %08x", i, plaintextes[i][0], plaintextes[i][1], plaintextes[i][2], plaintextes[i][3], result[0], result[1], result[2], result[3]);
			return XST_FAILURE;
		}
	}
